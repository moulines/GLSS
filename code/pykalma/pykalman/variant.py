...
DIM = {
    ...
    'transition_offsets': 2,
    'observation_matrices': 2,
    'observation_offsets': 2,
...
def _loglikelihoods(observation_matrices, observation_offsets,
                    ...
                    predicted_state_covariances, observations, controls):
    ...
    for t in range(n_timesteps):
        ...
        if not np.any(np.ma.getmask(observation)):
            ...
            observation_offset = _last_dims(observation_offsets, t)
            ...
            predicted_observation_mean = (
                ...
                + np.dot(observation_offset, control)
            )
            ...
            observation = np.ma.compressed(observation)
            predicted_observation_covariance = np.ma.compress_rowcols(
                predicted_observation_covariance
            )
            predicted_observation_mean = np.ma.compressed(
                predicted_observation_mean
            )
...
def _filter_predict(transition_matrix, transition_covariance,
                    ...
                    current_state_covariance, control):
    ...
    transition_offset : [n_dim_state, n_dim_control] array
    ...
    predicted_state_mean = (
        ...
        + np.dot(transition_offset, control)
    )
...
def _filter_correct(observation_matrix, observation_covariance,
                    ...
                    predicted_state_covariance, observation, control):
    ...
    observation_offset : [n_dim_obs, n_dim_control] array
...
def _filter(transition_matrices, observation_matrices, transition_covariance,
            ...
            initial_state_mean, initial_state_covariance, observations, controls):
    ...
    controls : [n_timesteps, n_dim_control] array
        controls from times [0...n_timesteps-1]. If `controls` is a masked 
        array and any of `controls[]`
    ...
    n_dim_control = controls.shape[1]
    ...
    for t in range(n_timesteps):
        if t == 0:
            ...
            transition_offset = _last_dims(transition_offsets, t - 1)
...
def _em(observations, controls, transition_offsets, observation_offsets,
    ...
    controls : [n_timesteps, n_dim_control] array
        control vector for times [0...n_timesteps-1].
    transition_offsets : [n_dim_state, n_dim_control] or [n_timesteps-1, n_dim_state, n_dim_control] array
        transition offset
    observation_offsets : [n_dim_obs, n_dim_control] or [n_timesteps, n_dim_obs, n_dim_control] array
        observation offsets
    ...
    transition_offsets : [n_dim_state, n_dim_control] array
        estimated transition offset
    observation_offsets : [n_dim_obs, n_dim_control] array
    ...
    if 'observation_matrices' in given:
        ...
        observation_matrix = _em_observation_matrix(
            observations, controls, observation_offsets,
    ...
    if 'transition_matrices' in given:
        ...
        transition_matrix = _em_transition_matrix(
            controls, transition_offsets, smoothed_state_means,
...
def _em_observation_matrix(observations, controls, observation_offsets,
                          smoothed_state_means, smoothed_state_covariances):
    ...
    .. math::
        ...
        C &= ( \sum_{t=0}^{T-1} (z_t - d_t u_t) \mathbb{E}[x_t]^T )
    ...
    for t in range(n_timesteps):
        if not np.any(np.ma.getmask(observations[t])):
            observation_offset = _last_dims(observation_offsets, t)
            res1 += np.outer(observations[t] - np.dot(observation_offset, controls[t]),
...
def _em_observation_covariance(observations, controls, observation_offsets,
    ...
    .. math::
        ...
        R &= \frac{1}{T} \sum_{t=0}^{T-1}
                [z_t - C_t \mathbb{E}[x_t] - d_t u_t]
                    [z_t - C_t \mathbb{E}[x_t] - d_t u_t]^T
                + C_t Var(x_t) C_t^T
    """
    _, n_dim_state = smoothed_state_means.shape
    n_timesteps, n_dim_obs = observations.shape
    ...
    for t in range(n_timesteps):
        if not np.any(np.ma.getmask(observations[t])):
            transition_matrix = _last_dims(observation_matrices, t)
            transition_offset = _last_dims(observation_offsets, t)
            err = (
                observations[t]
                - np.dot(transition_matrix, smoothed_state_means[t])
                - np.dot(transition_offset, controls[t])
            )
            res += (
                np.outer(err, err)
                + np.dot(transition_matrix,
...
def _em_transition_matrix(controls, transition_offsets, smoothed_state_means,
                          smoothed_state_covariances, pairwise_covariances):
    ...
    .. math::
        ...
        A &= ( \sum_{t=1}^{T-1} \mathbb{E}[x_t x_{t-1}^{T}]
                - b_{t-1} u_{t-1} \mathbb{E}[x_{t-1}]^T )
    ...
    for t in range(1, n_timesteps):
        ...
        res1 += (
            ...
            + np.outer(smoothed_state_means[t],
                       smoothed_state_means[t - 1])
            - np.outer(np.dot(transition_offset, controls[t - 1]), smoothed_state_means[t - 1])
        )
        res2 += (
            smoothed_state_covariances[t - 1]
...
def _em_transition_covariance(controls, transition_matrices, transition_offsets,
                              smoothed_state_means, smoothed_state_covariances,
                              pairwise_covariances):
    ...
    .. math::
        ...
        Q &= \frac{1}{T-1} \sum_{t=0}^{T-2}
                (\mathbb{E}[x_{t+1}] - A_t \mathbb{E}[x_t] - b_t u_t)
                    (\mathbb{E}[x_{t+1}] - A_t \mathbb{E}[x_t] - b_t u_t)^T
    ...
    for t in range(n_timesteps - 1):
        transition_matrix = _last_dims(transition_matrices, t)
        transition_offset = _last_dims(transition_offsets, t)
        err = (
            smoothed_state_means[t + 1]
            - np.dot(transition_matrix, smoothed_state_means[t])
            - np.dot(transition_offset, controls[t])
...
def _em_transition_offset(controls, transition_matrices, smoothed_state_means):
    ...
    .. math::
        ...
        b =  (\sum_{t=1}^{T-1}
                ...
                u_{t-1}^T)
             (\sum_{t=1}^{T-1} u_{t-1} u_{t-1}^T)^-1
    """
    n_timesteps, n_dim_state = smoothed_state_means.shape
    n_dim_control = controls.shape[1]
    res1 = np.zeros((n_dim_state, n_dim_control))
    res2 = np.zeros((n_dim_control, n_dim_control))
    for t in range(1, n_timesteps):
        transition_matrix = _last_dims(transition_matrices, t - 1)
        res1 += np.outer(
            smoothed_state_means[t] - np.dot(transition_matrix, smoothed_state_means[t - 1]),
            controls[t - 1]
        )
        res2 += np.outer(controls[t - 1], controls[t - 1])
    if n_timesteps > 1:
        return np.dot(res1, linalg.pinv(res2))
    else:
        return np.zeros(n_dim_state, n_dim_control)
...
def _em_observation_offset(observation_matrices, smoothed_state_means,
    ...
    .. math::
        ...
        d = (\sum_{t=0}^{T-1} (z_t - C_{t} \mathbb{E}[x_{t}])
            (u_t^T))
            (\sum_{t=0}^{T-1} u_t u_t^T)^-1
    """
    n_timesteps, n_dim_obs = observations.shape
    n_dim_control = controls.shape[1]
    res1 = np.zeros((n_dim_obs, n_dim_control))
    res2 = np.zeros((n_dim_control, n_dim_control))
    n_obs = 0
    for t in range(n_timesteps):
        if not np.any(np.ma.getmask(observations[t])):
            observation_matrix = _last_dims(observation_matrices, t)
            res1 += np.outer(
                observations[t] - np.dot(observation_matrix, smoothed_state_means[t]),
                controls[t]
            )
            res2 += np.outer(controls[t], controls[t])
            n_obs += 1
    if n_obs > 0:
        return np.dot(res1, linalg.pinv(res2))
    else:
        return np.zeros((n_dim_obs, n_dim_control))


class VariantKalmanFilter(object):
    ...
    .. math::
        ...
        x_{t+1}   &= A_{t} x_{t} + b_{t} u_{t} + \\text{Normal}(0, Q_{t}) \\\\
        z_{t}     &= C_{t} x_{t} + d_{t} u_{t} + \\text{Normal}(0, R_{t})
    ...
    observation_covariance : [n_dim_obs, n_dim_obs] array-like
        ...
        [0...n_timesteps-1]
    transition_offsets : [n_timesteps-1, n_dim_state, n_dim_control] \
    or [n_dim_state, n_dim_control] array-like
        Also known as :math:`b`.  state offsets for times [0...n_timesteps-2]
    observation_offsets : [n_timesteps, n_dim_obs, n_dim_control] or [n_dim_obs, n_dim_control] array-like
    ...
    n_dim_control: optional, integer
        the dimensionality of the control space. Only meaningful when you do 
        not specify initial values for `transition_offsets` or `observation_offsets`.
    """
    def __init__(self, transition_matrices=None, observation_matrices=None,
        ...
        n_dim_obs = _determine_dimensionality(
            [(observation_matrices, array2d, -2),
             (observation_offsets, array2d, -2),
             (observation_covariance, array2d, -2)],
            n_dim_obs
        )
        n_dim_control = _determine_dimensionality(
            [(transition_offsets, array2d, -1),
            (observation_offsets, array2d, -1)],
            n_dim_control
        )
        ...
        self.n_dim_control = n_dim_control

    def sample(self, n_timesteps, controls=None, initial_state=None, random_state=None):
        ...
        n_dim_control = transition_offsets.shape[-1]
        states = np.zeros((n_timesteps, n_dim_state))
        observations = np.zeros((n_timesteps, n_dim_obs))
        if controls is None:
            controls = np.zeros((n_timesteps, n_dim_control))
        ...
        for t in range(n_timesteps):
            if t == 0:
                ...
                states[t] = (
                    ...
                    + np.dot(transition_offset, controls[t - 1])
            ...
            observations[t] = (
                ...
                + np.dot(observation_offset, controls[t])
                + rng.multivariate_normal(
                    np.zeros(n_dim_obs),
        ...
        return (states, np.ma.array(observations), controls)

    def filter(self, X, controls=None):
        """Apply the Kalman Filter
        ...
        controls: [n_timesteps, n_dim_control] array-like
            control vectors corresponding to times [0...n_timesteps-1].  
        ...
        if controls is None:
            n_dim_control = transition_offsets.shape[-1]
            controls = np.zeros(n_dim_control)

        C = self._parse_observations(controls)
    ...
    def filter_update(self, filtered_state_mean, filtered_state_covariance,
                      control1, control2, observation=None, transition_matrix=None,
        ...
        filtered_state_covariance : [n_dim_state, n_dim_state] array
            ...
            times [1...t]
        control1 : [n_dim_control] array
            control vector at time t
        control2 : [n_dim_control] array
            control vector at time t+1
        ...
        transition_offset : optional, [n_dim_state, n_dim_control] array
        ...
        observation_offset : optional, [n_dim_obs, n_dim_control] array
        ...
        predicted_state_mean, predicted_state_covariance = (
            _filter_predict(
                ...
                filtered_state_covariance, control1
            )
        )
    ...
    def smooth(self, X, controls=None):
        """Apply the Kalman Smoother
    ...
    def em(self, X, controls=None, y=None, n_iter=10, em_vars=None):
        ...
        for i in range(n_iter):
            ...
            (self.transition_matrices,  self.observation_matrices,
             ...
             self.transition_covariance, self.observation_covariance,
             self.initial_state_mean, self.initial_state_covariance) = (
                _em(Z, C, self.transition_offsets, self.observation_offsets,
                    smoothed_state_means, smoothed_state_covariances,
    ...
    def loglikelihood(self, X, controls=None):
        ...
        loglikelihoods = _loglikelihoods(
          observation_matrices, observation_offsets, observation_covariance,
          predicted_state_means, predicted_state_covariances, Z, C
        )
    ...
    def _initialize_parameters(self):
        ...
        n_dim_state, n_dim_obs, n_dim_control = self.n_dim_state, self.n_dim_obs, self.n_dim_control

        arguments = get_params(self)
        defaults = {
            'transition_matrices': np.eye(n_dim_state),
            'transition_offsets': np.eye(n_dim_state, n_dim_control),
            'transition_covariance': np.eye(n_dim_state),
            'observation_matrices': np.eye(n_dim_obs, n_dim_state),
            'observation_offsets': np.eye(n_dim_obs, n_dim_control),
